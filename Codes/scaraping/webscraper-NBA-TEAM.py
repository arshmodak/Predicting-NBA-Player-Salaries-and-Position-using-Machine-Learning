...
def call_selenium_bs4(driver ,next = False, get_years = False):
...
def create_team_stats_table(driver):
    lst = call_selenium_bs4(driver,False,True)
    data_col = list()
    data_row = [data_col]
    for year in lst:
        driver.get("https://stats.nba.com/teams/traditional/?sort=W_PCT&dir=-1&Season="+year+"&SeasonType=Regular%20Season")
        time.sleep(2)
    ...
    all_player_stats.to_csv("D:\\Northeastern courses\\DS 5110\\Project\\Data-Management-and-Processing-Project\\datasets\\team_stats.csv",index=None, header=True)
